...
export function EditProjectModal({ open, setOpen, project, onSuccess }: EditProjectModalProps) {
  ...
  React.useEffect(() => {
    if (open && !prevOpen.current && project) {
      ...
      setAuthCredentials(
        project.authCredentials
          ? Object.entries(project.authCredentials).map(([key, value]) => ({ key, value }))
          : []
      );
      setPaymentCredentials(
        project.paymentCredentials
          ? Object.entries(project.paymentCredentials).map(([key, value]) => ({ key, value }))
          : []
      );
    }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="rounded-3xl">
        ...
        <DialogFooter>
          ...
          <Button variant="default" type="submit" form="edit-project-form" disabled={loading}>
