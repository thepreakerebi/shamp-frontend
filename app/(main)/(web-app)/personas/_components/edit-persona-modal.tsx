...
import { usePersonas, Persona } from "@/hooks/use-personas";
...
export function EditPersonaModal({ open, setOpen, persona, onSuccess }: EditPersonaModalProps) {
  ...
  React.useEffect(() => {
    if (open && !prevOpen.current && persona) {
      setForm({
        name: persona.name || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (persona) {
        await updatePersona(persona._id, {
          name: form.name,
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="rounded-3xl">
        ...
        <DialogFooter>
          ...
          <Button variant="default" type="submit" form="edit-persona-form" disabled={loading}>
